...
# function library (NET-04).
#
# Owns:
#   phase table     the phase table / intensity-gate / one-door-into-a-phase
#                    shape, mirrored from modules/dast/engine.sh one level
#                    down (the exact precedent this ticket was asked to
#                    follow).
#   HONESTY CONTRACT  the four-rule HONESTY CONTRACT: (1) an operator-configured
...
#
# THIS FILE ISSUES NO TRAFFIC, AND NEITHER DID run.sh AT THE TIME NET-04
# LANDED.  NET-04 shipped NO
# phase script, on purpose: zero
# phase scripts - every row was absent, every run recorded a declared
# reduction. This is DAST-02's shape and it is what made Tier 2
# parallelisable. A phase reaches the network exclusively through
...
# `http_request` for anything HTTP- or TLS-shaped - one TCP connect therefore
# costs exactly what one HTTP request
...
# 1. The phase table
# ---------------------------------------------------------------------------
# `<module-relative script>:<minimum run intensity>`, staged Tier 1's
# NET-05 through Tier 3's NET-10 - a deliberate "no CIDR/discovery/OS
# fingerprinting" boundary is what keeps this table short rather than
# open-ended. One row per ticket; `net_run_phase` treats an absent script as
# a clean no-op - the
# identical shape that lets modules/dast/engine.sh's own table be complete
# now rather than grown one edit at a time. Every row below is now landed.
#
# THE TIER IS A FLOOR, not a judgement made here:
...
# `httpport.sh` (NET-09) sit at `safe` (the `safe-active` type tag
...
# (NET-08) and `transport.sh` (NET-10) sit at `passive`, matching the
# `passive` type tag for NET-SVC-BANNER_DISCLOSURE-01/NET-TLS-*/
# NET-TRANSPORT-*.
#
...
declare -ga _NET_PHASES=(
  # Tier 1 - the authorised listener set.
...
# ---------------------------------------------------------------------------
# 4. The artifact-tuple scope pre-check
# ---------------------------------------------------------------------------
...
# operator-configured `config/scope.conf` tuple (a future phase calls that
# function directly and needs no wrapper
...
# or a cross-module inventory row - because any
...
# NOTED CHOICE (a lift into a shared home is preferred "ideally", not
# required, over forking a fourth copy).  Two reasons,
...
# dropped nothing.  A dropped row is recorded, never silent - the
# same "the filtered/not-open distinction is never collapsed"
...
# ---------------------------------------------------------------------------
# 6. The declared listener set (NET-05's own
