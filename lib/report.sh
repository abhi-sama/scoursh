...
declare -A _RPTC_RANSEM_TEXT=(
  ...
  [cloud]='Recorded AFTER the AWS call the check depends on returned an ANSWER - `ok` or `not_found` in lib/awscli.sh section 2s outcome vocabulary. A call that was denied, throttled, truncated or made against a region the account has not enabled is a declared coverage_reduction, listed below, never a silent checks_run entry: for a cloud scan an AccessDenied looks exactly like an account with nothing wrong in it, which is why this category classifies every failure rather than returning a status. NOTE: modules/cloud/aws/live/ ships the S3, API Gateway, KMS, Secrets Manager, SSM and IAM services so far (docs/STEP6-CLOUD-PLAN.md CLOUD-05/06/07/08/09, CLOUD-22); every other service in docs/DESIGN.md 8.1s catalog is still absent, so a --live run today resolves the account and its regions, examines S3, API Gateway, KMS, Secrets Manager, SSM and IAM, and records every other service as unexamined rather than counting it clean.'
)
