...
declare -A _RPTC_RANSEM_TEXT=(
  ...
  [cloud]='Recorded AFTER the AWS call the check depends on returned an ANSWER - `ok` or `not_found` in lib/awscli.sh section 2s outcome vocabulary. A call that was denied, throttled, truncated or made against a region the account has not enabled is a declared coverage_reduction, listed below, never a silent checks_run entry: for a cloud scan an AccessDenied looks exactly like an account with nothing wrong in it, which is why this category classifies every failure rather than returning a status. NOTE: modules/cloud/aws/live/ ships the S3, Lambda, RDS, DynamoDB, API Gateway, KMS, Secrets Manager, SSM, IAM, EC2/VPC, CloudTrail, AWS Config, GuardDuty, Inspector2 and Macie2 services so far (docs/STEP6-CLOUD-PLAN.md CLOUD-05, CLOUD-21, CLOUD-15/16, CLOUD-06/07/08/09, CLOUD-22, CLOUD-13, CLOUD-30..34); every other service in docs/DESIGN.md 8.1s catalog is still absent, so a --live run today resolves the account and its regions, examines those fifteen services, and records every other service as unexamined rather than counting it clean.'
)
