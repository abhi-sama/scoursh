...
## Status: Track A (SARIF-01 through SARIF-06) is complete; Track B (COMPLIANCE-01 through COMPLIANCE-04) is now complete too
...
| SARIF-04 - `runs[0].results[]`: the per-finding mapping | landed |
| SARIF-05 - make the §12 SARIF and JSON validation real | landed |
| SARIF-06 - SARIF documentation, and correcting the stale `--format` prose | landed |
| COMPLIANCE-01 - the OWASP category label table and its expansion | landed |
| COMPLIANCE-02 - the OWASP compliance view in `report.md` and `report.html` | landed |
| COMPLIANCE-03 - `data/cis-mappings`: the format, the vendored file, and its refresh procedure | landed |
| COMPLIANCE-04 - the CIS compliance view in `report.md` and `report.html` | landed |
...
`_sarif_build_registry` runs DIRECTLY inside `report_sarif`, never through `$(...)`, for the identical reason `checks_registry_load`'s own header gives: its `die()` on a malformed registry file must abort the run, and a `die()` inside a command substitution does not reliably do that. Sourcing `lib/checks.sh` from `lib/report.sh` is a real diamond: `modules/dast/engine.sh` already carries its own real edge to `lib/checks.sh`, so any entry point reaching both `lib/report.sh` and a DAST module (`tests/suites/dast-methods.sh`, measured) re-expands `lib/checks.sh`'s own dependency chain a second time under `shellcheck -x`, which took that file from hub sum 17 to 19 against `tests/lint-source-graph.sh`'s cap of 17. The fix is the standard `# shellcheck source=/dev/null` back-edge cut on `lib/report.sh`'s own edge to `lib/checks.sh` - lossless for the runtime `source` (its `SCOURSH_CHECKS_SOURCED` guard makes a repeat a no-op either way) and, checked directly rather than assumed, lossless for `shellcheck -x` too: `lib/report.sh`, `tests/suites/report.sh`, `sarif-locations.sh` and `sarif-rules.sh` (the only entry points that need `lib/checks.sh`'s declarations from THIS edge, since none of them reaches it any other way) were each shellchecked standalone before and after the cut with no new finding.
...
**SARIF-04 has also landed.** `runs[0].results[]` is now the field-by-field mapping this document's own table specifies, in a new `lib/report.sh` section 5b: `_sarif_result_location` (the four-case location table, re-running the identical `_locations_history_resolves` filesystem test SARIF-02 already made rather than trusting a separate marker), `_sarif_message_for` (evidence to `region.snippet.text` when a region exists, else a `message.text` continuation), and `_sarif_print_one_result`/`_sarif_print_results` (the rest of the mapping - `ruleId`, `level` via the same `_sarif_level_for` SARIF-03's rule descriptors already use, `partialFingerprints`, `properties`, `suppressions[]`). Every finding is read through `finding_decode`, never a hand-rolled parse of `findings.fields`, keeping the emitter inside the redaction guarantee. `security-severity` is deliberately never emitted, for the reason this document's own "the severity trap" section gives. The SCA severity-provenance gap is recorded rather than guessed around: `data/advisories.db` cannot distinguish a genuinely medium-rated advisory from the unscored fallback (both land on `medium`), so `result.properties.severityProvenance` is never emitted for any SCA finding, and a `coverage_reduction` fact is written into `run.json` instead, once per run directory and only when a `medium`-severity SCA finding is actually present - `critical`/`high`/`low` can never be the fallback, so nothing is recorded for those. `tests/suites/sarif-results.sh` (49 assertions) is the proof, including this ticket's own two named acceptance criteria (no `result.ruleId` lacking a descriptor across a run emitting five profiles and both ungoverned id families at once; a redacted secret, and the tension-10 hostile-evidence fixture, staying redacted and JSON-valid in `report.sarif`) and every case from the mapping table, the location table, and the level table. Landing it surfaced a latent, previously-invisible bug in `sarif-rules.sh`'s own determinism test - a missing `occurrence_reset_all` between its two fixture runs, which only produced a *visible* fingerprint disagreement once `partialFingerprints` gave `results[]` something to disagree on - fixed in the same change, alongside normalising `results[]`'s own wall-clock `firstSeen`/`lastSeen` before comparing.

**SARIF-05 has also landed.** Both halves. **(a)** `tests/fixtures/sarif/sarif-schema-2.1.0.json` vendors the OASIS SARIF 2.1.0 schema (errata01, the standards body's own hosted copy - the `oasis-tcs/sarif-spec` GitHub URL `report_sarif`'s own `$schema` field cites has since had its default branch renamed and no longer resolves at that path, so the vendored copy is not only offline but the more authoritative source now too; provenance and refresh procedure in `tests/fixtures/sarif/README.md`). `tests/lib/sarif_validate.py` is a small, purpose-built validator over exactly the draft-04/07 keywords that one schema uses (`type`, `enum`, `$ref`, `properties`, `additionalProperties`, `required`, `items`, `minItems`, `maxItems`, `uniqueItems`, `minimum`, `maximum`, `pattern`, `anyOf`, `oneOf`) rather than a dependency on the `jsonschema` PyPI package, because that package is not part of the standard library and is not installed by default even where `python3` is - depending on it would have reintroduced exactly the skip-as-pass failure this ticket exists to close, on nearly every host rather than the rare one. It also implements tension 22's own strengthened extra condition (`--check-locations RUNDIR SCANROOT`): every `results[*].locations[0].physicalLocation.artifactLocation.uri` must be non-empty and resolve to a real file under the run directory or the scanned tree, checked with `os.path.exists`, never by re-trusting the emitter's own string. `tests/suites/sarif-schema.sh` (14 assertions) is the proof: one fixture run deliberately covering all four location-table cases, both a registry-backed and a synthesised rule descriptor, a suppressed finding, and the tension-10 hostile-evidence fixture, validates cleanly against the vendored schema with the location condition also checked - a trivial one-result document would validate under any reading, including a validator that does nothing, so the fixture is built rich enough that a real defect in any of those shapes would fail it. Four deliberately malformed documents (a missing required key, a property `additionalProperties: false` forbids, an out-of-enum `level` value, and a `uri` resolving to nothing) are each proven still well-formed JSON and each proven rejected, which is the demonstration that a well-formedness-only check - this project's own pre-SARIF-05 defect - would have passed every one of them. **(b)** `tests/suites/report.sh`'s `python3`-absent branch, which called `_t_ok 'python3 unavailable, JSON schema validation skipped'` and so reported unrun validation as a pass, now prints a loud `NOTICE ... This is a SKIP, not a pass.` and records nothing, matching the convention `tests/suites/dast-tls.sh`, `tests/suites/netns.sh` and `tests/suites/paranoid.sh` already use for the identical shape. Verified both ways: run with `python3` on `PATH`, the suite's real assertion count is unchanged; run with a `PATH` built to contain every other tool this suite needs but no `python3` anywhere on it, the suite still exits 0, prints both `NOTICE` lines, and its passed-count is honestly lower rather than inflated by the removed fake pass.

**SARIF-06 has now landed, completing Track A.** Documentation only, per its own row's `Notes` column: it added an operator-facing "SARIF output" section to `docs/USAGE.md` covering what `report.sarif` carries (`tool.driver.rules[]`, `results[]`, both the physical and logical location on every result, `partialFingerprints["scourshFingerprint/v1"]`), what it deliberately never carries (`security-severity` - "the severity trap" above, restated for an operator rather than an implementer - plus `codeFlows`/`fixes[]`/`result.rank` and the reasons), how `partialFingerprints` lets a consumer track a finding across runs while `status` reads `new` on every result and `suppressions[]` stays empty until step 7's baseline suppression lands, and how a CI step points at `report.sarif` today (`--format sarif`, or add `sarif` to a multi-value `--format`/`formats` list). It replaced the three stale "no SARIF writer" statements this ticket's own row names - `README.md`'s "Known gaps" entry, `docs/USAGE.md`'s global-flag table row for `--format`, and `docs/USAGE.md`'s `--format`/`formats` writeup, which is now the new SARIF output section rather than a stub under "Accepted but not yet implemented" - and, found by searching the tree rather than trusting this plan's own list per this ticket's own instructions, four more: an identical "does not exist yet" phrase in `AGENTS.md` and in `docs/FOUNDATION.md` (tension 9's redaction-backstop entry, which the two files carry as near-duplicate prose), `AGENTS.md`'s and `docs/FOUNDATION.md`'s own step-10 status paragraphs (which this same change updates to say Track A, docs included, is complete), and `ROADMAP.md`'s "Not yet started"/"Known defects" entries for `--format sarif`'s previously-empty `results[]`. `docs/USAGE.md`'s `formats` config-key row was also corrected from `inert` to `live` (`scan.sh` resolves it through `config_scanner_list` at `_scan_capture_list SCOURSH_FORMATS config_scanner_list formats ...`, confirmed by reading `scan.sh`/`lib/config.sh` rather than assumed) since it was already stale independently of SARIF and this ticket's own scope is "correcting the stale `--format` prose", not only its SARIF half. Two places were read and deliberately left alone: `docs/DESIGN.md` stays verbatim per this project's own rule, and `lib/report.sh`'s and `lib/findings.sh`'s own source comments still say "a SARIF emitter that does not exist yet" in two places (`lib/report.sh:873`, `lib/findings.sh:1161`) and `tests/suites/scan.sh`'s `--format sarif` test case carries a stale description and asserts nothing about `report.sarif` itself - all reported as findings rather than fixed here, since this ticket's scope is documentation only.

**COMPLIANCE-04 has now landed, completing Track B and step 10 in full.** It was unblocked the moment `modules/cloud/aws/live/s3.sh` merged (the first `cis`-carrying finding this view needed to build and test against), and mirrors COMPLIANCE-02's OWASP view exactly in shape: `lib/report.sh` section 1c (`_report_cis_registry_load`/`_report_cis_state`/`_cis_bucket`/`_cis_render_order`, alongside `_RPT_CIS`) is the CIS twin of section 1a's own OWASP mechanics, and `_md_cis_compliance`/`_html_cis_compliance` (called from `report_md`/`report_html` right after their OWASP siblings) are the CIS twins of `_md_owasp_compliance`/`_html_owasp_compliance`. `cis_mappings_load` (COMPLIANCE-03) gained one small, additive extension: `_CIS_ORDER`, an indexed array of ids in on-disk (natural benchmark-numbering) order, because a `cis` id is dotted-decimal and NOT zero-padded (`1.10` sorts lexically before `1.2`) - the `LC_ALL=C` re-sort `_owasp_render_order` relies on is provably wrong here and would render the benchmark's own numbering out of order.

`cis` is `optional, repeatable` where `owasp` is `required, single` (rules/RULE-FORMAT.md §9.1/§9.5), so the CIS view is not a byte-for-byte template substitution of the OWASP one: there is no `none`-mapped tail section (a finding with no `cis` value simply belongs to no control group, the ordinary case for nearly every check in this build), and the registry map is check_id -> a newline-joined LIST of ids rather than one scalar. The ticket's own mandatory honesty split adds a FOURTH state beyond OWASP's three (findings/clean/out-of-scope) plus its own extra two (filtered/not-run): **`not_applicable`** - "this control's check(s) ran but found no matching resource in the scanned account" - which OWASP's per-category view has no equivalent of, since only a cloud/posture check can produce that fact. It is read from `meta/coverage_reduction`, extended to recognise BOTH the bracketed `checks=[A B C]` shape `_report_coverage_state` already reads for other modules AND the singular, unbracketed `check=<id>` shape `modules/cloud/aws/live/s3.sh`'s own per-account roll-up (`_s3_record_coverage`) actually emits - proven against that exact shape, not an invented one, since a check named that way is by construction never also in `checks_run`.

`tests/suites/report.sh`'s own COMPLIANCE-04 case adds a fifth fixture registry, `tests/fixtures/checks-registry/modules/cloud/aws/live/checks.rules` (registered in `tests/lint-rules.sh`'s `fixture_schema_for`, sibling to the existing DAST one), whose `cis:` values are real CIS AWS Foundations Benchmark v3.0.0 ids from the shipped `data/cis-mappings` table - proving real labels and the real benchmark version render (not only the degrade-visibly path an unknown id already covered under COMPLIANCE-03), while its check IDs are entirely fixture-only per the DAST fixture's own E019 warning. All four bucket states plus out-of-scope render distinctly against one run: 2.1.4 (a live finding), 2.1.1 (clean), 1.6 (not applicable, via the s3.sh-shaped `coverage_reduction`), 1.8 (filtered by `--profile-scan`/`--intensity`), and 1.5 (out of scope - no fixture check cites it). Operator docs (`README.md`, `ROADMAP.md`) are updated in the same change to say the compliance report is complete, per this ticket's own "Carries its own operator documentation, per §13 item 10's 'docs/README'" row.
...
|---|---|---|
| **SARIF 2.1.0 emitter** (Track A) | **Complete** - SARIF-01 through SARIF-06 have all landed | Nothing. Not step 6, not step 7. |
| **Compliance-mapping report** (Track B) | **Complete** - COMPLIANCE-01 through COMPLIANCE-04 have all landed | Nothing. |
| **`--fail-on` CI gate** | **Already shipped in full** | Nothing. No step 10 ticket exists for it. |

The consequence is that step 10 was never one thing waiting behind step 6 and step 7.
Track A (six tickets) has landed in full, ahead of both, and touched none of the files either would have needed.
Track B's first three tickets landed similarly unblocked, and its fourth (COMPLIANCE-04) landed the moment step 6 supplied its first `cis`-carrying finding (`modules/cloud/aws/live/s3.sh`) - it never needed the rest of step 6 to follow.
**Step 10 is therefore complete in full.**

`ROADMAP.md` currently places step 10 second in priority behind step 7 - that ordering governed when Track A was picked up, not whether it could ship, and it has now shipped regardless of the ordering. This plan does not change that ordering for the remainder of step 10 (Track B), and does not argue for changing it: it records that the ordering is a priority choice rather than a technical block.
...
What SARIF-06 owned was the last sentence of each: the statement that no SARIF writer exists, which was true only until Track A landed. It has now landed in full, and SARIF-06 corrected that sentence everywhere it appeared - see its own landing paragraph above for the full list of files, which turned out to be larger than the three this row named.
...
**This section is a historical account of why the split existed, not the current state.**
`modules/cloud/aws/live/s3.sh` has since landed and carries real `cis` values (§8.1's own requirement, above), which is what unblocked COMPLIANCE-04 - see this document's own Status table and the "Landed once step 6 supplied its first `cis`-carrying finding" row below for the outcome.
...
| # | Ticket | Depends on | Implements | Notes |
|---|---|---|---|---|
| **COMPLIANCE-01** (landed) | The OWASP category label table and its expansion | nothing | `rules/RULE-FORMAT.md` §9.1's unimplemented "The report expands it to the full label" | A vendored, auditable data table mapping each `A<nn>:<yyyy>` id to its published category name, read from disk rather than compiled into a `case` statement, for the same reason `data/severity-rubric.conf` is data: it is reviewable and diffable. Covers the OWASP Top 10 2021 set that `E026`'s pattern admits and that Appendix B names. An id with no row renders as the bare id plus a recorded reason, never as a blank or an invented label - a category the table has never heard of and a category with no findings must not look the same. `none` is a legal value on a real check and is not a missing label; it renders as "not categorised" and is counted separately. Tests assert both directions: a known id expands, an unknown id degrades visibly. |
| **COMPLIANCE-02** (landed) | The OWASP compliance view in `report.md` and `report.html` | COMPLIANCE-01 | `docs/DESIGN.md` §4's "compliance view"; Appendix B | Groups the **findings** by category, not merely counts them, which is what makes it a compliance view rather than the summary table `lib/report.sh` already renders; the existing `_RPT_OWASP` count table is kept and gains the label. Adds the equivalent section to `report.md`, which has none today. Renders Appendix B's coverage statement honestly alongside it - A04 not covered, A08/A09/A01 partial - because a compliance view that lists ten categories with findings under seven of them implies the other three are clean, and Appendix B exists precisely to say two of those three were never in scope. A category that is in scope and genuinely produced no finding, a category that is out of scope by design, and a category whose checks were filtered out of this run by `--profile-scan`/`--intensity` (tension 15) are three different facts and must render as three different things; `checks_run` and the run's `coverage_reduction` records are what distinguish them and are already written. HTML stays self-contained with the inline CSP and no `<script>`, per tension 10. |
| **COMPLIANCE-03** (landed) | `data/cis-mappings`: the format, the vendored file, and its refresh procedure | nothing | `docs/DESIGN.md` §3 and §4's named path | Specifies and creates the file **as a control-id-to-label reference table, not as a source of control ids** (see above). Format follows `data/versions.db`'s precedent, and the choice between the frozen `rules/RULE-FORMAT.md` record format and a TSV is made by the same rule that already governs it: human-authored files use records, machine-generated files use JSON or the frozen TSV. Since this table is transcribed from a published benchmark PDF by a human, it is a **records** file. Carries the benchmark name and version, because "CIS AWS Foundations Benchmark v3.0.0 control 1.4" and "v1.4.0 control 1.4" are different controls and a table that cannot say which it is silently misattributes findings. Ships a documented refresh procedure in the shape `docs/VERSIONS-DB.md` §5 already uses for the `banner` namespace, because a list nobody can refresh becomes wrong quietly. **This ticket is unblocked and lands the format and the table; it renders nothing.** It is separable from COMPLIANCE-04 for exactly that reason, and landing it early means step 6's own CLOUD-0x tickets have a table to check their `cis:` values against as they are written, rather than after. **Landed as:** `rules/RULE-FORMAT.md` §9.6.7 (the schema: `id`/`title`/`benchmark`/`benchmark-version`/`format-version`, the last three meaningful only on the first record - the identical convention `format-version` already uses in §9.6.5/§9.6.6); `data/cis-mappings` itself, a 34-control seed for CIS AWS Foundations Benchmark v3.0.0 covering the v1(CIS-core) IAM/S3/EC2-VPC scope plus its logging/monitoring dependents (CloudTrail, AWS Config, KMS, EFS, RDS), cross-referenced against AWS Security Hub's own public v3.0.0 control-id mapping rather than reproducing the licensed benchmark PDF's rationale/audit prose - Section 4 (the CloudWatch alarm controls) is a stated, documented gap rather than a guess, since Security Hub's own mapping does not name their v3.0.0 numbers; `docs/CIS-MAPPINGS.md`, the normative refresh-procedure document mirroring `docs/VERSIONS-DB.md`'s shape; and `lib/report.sh`'s `cis_mappings_load`/`cis_control_label`/`cis_control_known`/`cis_benchmark_name`/`cis_benchmark_version`, unwired into `report_all`/`report_md`/`report_html` - the identical "lands the lookup, not the section" shape COMPLIANCE-01 landed ahead of COMPLIANCE-02. No `format_version` bump: a wholly new path resolving to a wholly new schema trips only `rules/RULE-FORMAT.md` §14 item 2 (`lib/records.sh` and `tests/lint-rules.sh` gain the new schema/path arm), per that section's own "additive path-table row" worked example - no existing check id, module prefix, or fingerprint input is touched. |

#### Landed once step 6 supplied its first `cis`-carrying finding

| # | Ticket | Depends on | Implements | Notes |
|---|---|---|---|---|
| **COMPLIANCE-04** (landed) | The CIS compliance view in `report.md` and `report.html` | COMPLIANCE-02, COMPLIANCE-03, **and `modules/cloud/` existing** | `docs/DESIGN.md` §4's CIS half; §8.1's "every finding cites ... the CIS control id" | Groups findings by CIS control id, expanded through COMPLIANCE-03's table, and states the benchmark name and version at the head of the section. Was blocked on step 6, and specifically on the first CLOUD-0x or POSTURE-0x ticket that emits a finding carrying a `cis` value; unblocked the moment `modules/cloud/aws/live/s3.sh` landed, exactly as this row's own earlier text predicted ("one landed cloud check is enough to build and test against"). Mirrors COMPLIANCE-02's honesty requirement, with a fourth state its OWASP sibling has no equivalent of: a control with no finding because it was assessed and passed, a control with no finding because its check has not been written, a control whose check ran but found no matching resource in the scanned account, and a control excluded from this run by `--profile-scan`/`--intensity` are four different facts and render as four different things. **Landed as:** `lib/report.sh` section 1c (`_report_cis_registry_load`/`_report_cis_state`/`_cis_bucket`/`_cis_render_order`, `_RPT_CIS`), the CIS twin of section 1a's own OWASP mechanics; `_md_cis_compliance`/`_html_cis_compliance`, called from `report_md`/`report_html` right after their OWASP siblings; and one small, additive extension to COMPLIANCE-03's `cis_mappings_load` (`_CIS_ORDER`, an on-disk-order indexed array, since a dotted-decimal `cis` id is not zero-padded and a plain `LC_ALL=C` sort - correct for OWASP's own zero-padded ids - would render `1.10` ahead of `1.2`). The "not applicable to the account" state reads `meta/coverage_reduction`, recognising both the bracketed `checks=[A B C]` shape other modules use and the singular `check=<id>` shape `modules/cloud/aws/live/s3.sh`'s own per-account roll-up actually emits. Carries its own operator documentation (`README.md`, `ROADMAP.md`), per §13 item 10's "docs/README". |
