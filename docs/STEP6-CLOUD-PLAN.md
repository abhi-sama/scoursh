...
up until every earlier `docs/DESIGN.md` §13 step is complete on `dev`.**
The build order (§13) is strictly sequential, so step 6 waits behind the tail of step 3, all of step 4,
and all of step 5 - not just step 4 alone, even though step 4 is the step immediately before it and the
one this ticket's own acceptance criteria call out by name:

1. **§13 step 3 (SAST) finishes**: the `nosql` and `ldap` rule packs
   (`modules/sast/rules/nosql.rules`, `modules/sast/rules/ldap.rules`) land.
   As of this writing `dev` has shipped `secrets`, `crypto`, `injection`, `python`, `go`, `javascript`,
   `java`, and `history.sh` (sub-steps 3a-3e); only `nosql`/`ldap` remain, per `AGENTS.md`'s "Current
   position".
2. **§13 step 4 (SCA + IaC) completes.**
   As of this writing this is only partly true: the IaC half has one ticket landed on `origin/dev`
   (`modules/iac/run.sh`, `parse.sh`, `terraform.rules` - "IaC: Terraform checks via the pattern-rule
   engine", commit `5de4460`, one commit ahead of this branch's base `202e3d5`), but the SCA half
   (lockfile parse -> `data/advisories.db` matching, `modules/sca/`) has not started at all - no
   `modules/sca/` directory exists on `dev` or `origin/dev`. Step 4 as a whole is therefore **not**
   complete, and this is the AC-named gate: this ticket's acceptance criteria require the plan to say so
   explicitly, matching the precedent `docs/STEP5-DAST-PLAN.md` already set for step 5's DAST-0x
   tickets.
3. **§13 step 5 (DAST) completes.**
   As of this writing DAST has only a written plan (`docs/STEP5-DAST-PLAN.md`); zero of DAST-01 through
   DAST-30 has landed and `modules/dast/` does not exist. Step 6 comes after step 5 in the same
   sequential order, so it is gated on step 5 too, independent of whatever step 4's status is by the time
   step 5 finishes.
...
`tests/lint-aws-readonly.sh` (154 lines) already exists on `dev`, landed at §13 step 1
...
longer appears in the code is a failure. Its own header says plainly: *"`lib/awscli.sh` and `aws/live/`
arrive at the start of §13 step 6. Nothing exists for this lint to examine yet, so it passes over an
empty set - and says so, rather than reporting a green it did not earn."* `tests/aws-readonly-allow.txt`
itself does not exist yet either; the lint prints a note that it is "seeded at §13 step 6".

So the "standalone ticket for the read-only-verb CI lint" this plan's acceptance criteria require
(CLOUD-03 below) is **not** "write the lint" - that already happened. It is: land `lib/awscli.sh`'s
`aws_ro` chokepoint for the lint to have something real to examine, seed `tests/aws-readonly-allow.txt`
with the two entries tension 23 names (`sts assume-role`, `sts get-caller-identity`), and add the
negative-fixture test tension 23's own "Consequence for the build" paragraph calls for: a script with a
mutating `aws_ro` call, asserted to fail **both** the lint and `aws_ro`'s runtime guard (exit `3`).
...
`docs/DESIGN.md` §13 step 4 reads "**SCA** ... and **IaC** (cloud + container rules)", and the IaC ticket
that already landed (`modules/iac/run.sh` + `terraform.rules`, above) is exactly that: a static,
offline, pattern-rule-engine parse of `.tf`/CloudFormation files, with no `aws_ro` call and no dependency
on live credentials at all. §8.2 lives under the "8. Module - Cloud / AWS (live + IaC)" heading only
...
|---|---|---|---|
| CLOUD-01 | `lib/awscli.sh` - the `aws_ro` read-only chokepoint (`docs/FOUNDATION.md` tension 23) | `lib/core.sh` (`scan_match`, scratch-dir mutex primitives, shipped step 1); `lib/http.sh` as the sibling chokepoint precedent (shipped) | `aws_ro <service> <operation> [args...]` validates `<operation>` against the frozen prefix allowlist (`^(describe\|list\|get\|search\|lookup\|select\|head\|batch-get\|preview\|estimate\|simulate)(-\|$)`) before executing; a non-matching operation aborts with exit `3`, logged as a scope violation - same class as an out-of-scope host. Pins `--no-cli-pager --output json`; never accepts `--cli-input-json`/`--cli-input-yaml` from a caller. Owns the per-`(service, region, account, operation, args)` response cache (§10, tension 16's note) so multiple checks reading the same `describe-*` do not re-fetch. Seeds `tests/aws-readonly-allow.txt` with `sts assume-role` (needed by CLOUD-02's `--assume-role`) and `sts get-caller-identity` (already covered by the prefix, listed for clarity), per tension 23 item 4. |
