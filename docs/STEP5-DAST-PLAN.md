...
The guided mode (`docs/STEP-GUIDE-PLAN.md`) may *offer to write a record*; it never answers the gate,
and the gate then re-reads the file and can still refuse.
...
| circuit breaker | **partially** | TWO thresholds since the breaker-5xx-semantics fix (`docs/FOUNDATION.md` tension 16's amendment): `circuit-breaker-failures` for transport-level failures and the much-higher `circuit-breaker-5xx-failures` for well-formed 5xx responses, over the same window. Both raisable, neither disableable, and the argument is unchanged: disabling has no upside even on your own host, because a target returning sustained 5xx on either counter produces no useful findings, so continuing to hammer it buys nothing. There is no honest case for a prompt that removes either. |
...
estimate for step 5.
One later section adds work on top of those 30: DAST-31 through DAST-36 ("Safety defaults and
authorisation", above).
A second body of work, GUIDE-01 through GUIDE-07 ("the guided interactive mode"), was originally
designed inside this document too, but it was never actually DAST-specific - it covers `sast`, `sca`,
`iac` and `cloud` as well as `dast`, and GUIDE-01 through GUIDE-03 are not DAST-gated at all - so it now
has its own plan document, `docs/STEP-GUIDE-PLAN.md`; see that document rather than this one for its
design and its per-ticket table.
Neither of these two sections renumbers anything here.
...
## The guided interactive mode has its own plan document

GUIDE-01 through GUIDE-07 - a guided mode that asks the operator what to scan and composes the
equivalent flags, with, for DAST, an affirmation flow before the conservative request limits are
lifted - was originally designed in this section.
It was never actually DAST-specific: it covers `sast`, `sca`, `iac` and `cloud` as well as `dast`, and
GUIDE-01 through GUIDE-03 carry no DAST dependency at all.
An audit of this project's status documents (2026-09-02) found that burying cross-cutting work inside a
DAST-titled plan made it undiscoverable from `ROADMAP.md`, `AGENTS.md` or `docs/DESIGN.md`, so the full
design and its per-ticket table now live in `docs/STEP-GUIDE-PLAN.md` - see that document rather than
this one for the flow, the flag equivalence table, the audit-record design, the measured `select`
behaviour, and the GUIDE-01 through GUIDE-07 tickets themselves.
