...
Of the six ecosystems `docs/DESIGN.md` §6.5 names, only **Go** (`go.mod`/`go.sum`) is still open (that
sentence read "Go, Java, and PHP ... are still open" until `ab23b79`); step 4's SCA half is not complete.
