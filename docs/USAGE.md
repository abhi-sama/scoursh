...
**Do not commit the resulting `bin/`/`rules/` directories to git** - `modules/*/adapters/*/bin/` and
`.../rules/` are gitignored by design; re-run the vendor step per machine or per CI image instead of
checking the bytes in:
...
Every flag below parses and validates today, but they are not all inert: only `--lang` is a pure
no-op. The rest gate real, live behaviour in the commands named in their own subsection (`--intensity`/
`--allow-intrusive` on `dast`/`network`, the DAST rate/budget/breaker ceilings, the identifying
User-Agent, and `sca`'s advisory-database gate) - they are grouped here because each one's effect is
either absent outside a specific command, conditional, or narrower than its name suggests, and each
subsection states precisely which. Read each subsection's own text for what it actually does before
assuming "not yet implemented" applies to it.
...
runs every phase `--intensity` admits (NET-01 through
