...
- **`.github/workflows/ci.yml` RUNS. The repository is public now, but a `pull_request` run is Ubuntu-only.**
  ...
  By later, maintainer-directed change, the `suite` job's `strategy.matrix.include` is itself built from `github.event_name`: a `pull_request` run gets the `ubuntu-latest` entry alone, so the `macos-latest` leg is never generated as a job at all on that trigger - not generated and skipped, which is a difference that matters here, since `matrix` is not one of the contexts a job-level `if:` can read (`github`, `inputs`, `needs`, `vars` only; `actionlint` catches a job `if:` that references `matrix`). That leg (~2.5h) was the pole every PR waited on while the maintainer merges on the Ubuntu leg (~1.5h) alone. A `push` to `main` or `dev` still gets both legs, so a macOS-only regression is caught on `dev` before it is promoted to `main` - just after merge, not before it. The `compare` job (below) follows the same rule: it needs both legs' findings, so it carries `if: github.event_name != 'pull_request'` and is skipped (not failed) on a `pull_request` run.
...
In CI (`GITHUB_ACTIONS=true`), the stage runs **one `shellcheck` invocation per file**, `sc_ci_jobs` of them in parallel, protected by the SAME real-resident-memory watchdog the local path uses (`_sc_run_pass`, shared between the two branches) - never a `ulimit` of any kind, and never a cgroup.
...
**A follow-up then bound each per-file invocation with `ulimit -v` (RLIMIT_AS), and that was itself a defect, since corrected.**
`ulimit -v` bounds *virtual address space*, and GHC's RTS reserves address space in an amount that scales with how much memory the *host* appears to have, not with the file being checked - so a ceiling low enough to protect a ~15GB runner rejected eight real files outright (`shellcheck` exiting 251, "out of memory") at 10, 12, 14 **and** 16GB, despite none of them needing anywhere near that much *resident* memory. A real per-process memory cgroup (`docker run --memory`, `systemd-run --scope -p MemoryMax=`) was investigated and rejected for the opposite reason: cgroup `memory.max` does not stop GHC from *reserving* however much virtual space it wants (a reservation is not a fault until the pages inside it are dirtied), so nothing gives GHC's heap-growth heuristic any earlier back-pressure - it grows exactly as it would unconstrained, right up to the moment the kernel's cgroup OOM killer SIGKILLs it, with no chance to shrink first. Measured directly: this tree's own heaviest file, run inside a container with a real 10GB memory limit on a host with ~15GB total, was killed (exit 137) after running for **over six minutes** - the same `ulimit -v 10GB` bound left that exact file finishing cleanly, because the virtual ceiling forces smaller allocations to fail fast and GHC's allocator adapts to what it is given, where a real-memory cgroup lets it commit to a doomed trajectory before anything intervenes.

The fix reuses the local path's own external, polling watchdog instead: it never constrains what GHC may *reserve* (so a large, legitimate startup reservation for a heavy `-x` closure is never mistaken for an overrun), and it acts on real, sampled RSS rather than waiting for the kernel's own OOM killer to notice - the same margin that already protects a contributor's own machine, where no `ulimit` or cgroup of any kind is available at all. One property is still deliberately **not** the local path's:

* **No `skipped` outcome.** "This host is too small for this file" is a legitimate answer about a contributor's laptop and is never a legitimate answer on CI, where the runner *is* the target. A file that cannot be checked on CI - whether the watchdog's cause was OVER BUDGET or HOST MEMORY PRESSURE - lands in `sc_unchecked` and **fails** the stage. That is what keeps a green CI run from meaning "every file we felt like checking was clean".

`sc_ci_jobs` is derived from the runner's own available memory (clamped to the core count, and to 4) rather than pinned at 2, because the two runners differ by more than 2x in RAM and one constant cannot be right for both.
The arithmetic mirrors the local model's two-pass, `reserve`/`headroom` shape exactly, because it now **is** the local model's `_sc_run_pass`: pass 1 plans wide against a **typical** footprint (`SCOURSH_SHELLCHECK_CI_WORST_GB`, default 6GB - measured worst real resident size on this tree with no virtual-space cap artificially shrinking it), and a file that exceeds it is *deferred to pass 2*, which retries it alone against the whole of the runner's real headroom rather than refusing it outright. That second tier is what the old `ulimit -v` model never had, and is why raising one fixed ceiling could never have fixed the eight-file regression without also reopening the false-pass risk `ulimit -v` was introduced to close.

| runner | total | avail | reserve | headroom | pass 1 plan | pass 1 typical budget |
|---|---|---|---|---|---|---|
| `ubuntu-latest` | 16GB | ~15GB | 2GB | 13GB | **2** x 1 file | 2 x 6 = 12GB, inside 13GB |
| `macos-latest` | 7GB | ~5GB | 2GB | 3GB | **1** x 1 file | 6GB clamped to the 3GB headroom - one file at a time is as narrow as this can be made without dropping a file, which is not on offer |

A file pass 1 defers on either shape is retried alone in pass 2 at the *whole* of that runner's headroom (13GB and 3GB respectively) before it is ever allowed to fail the stage.
...
`tests/suites/run-tests-stage.sh` section **M** covers the one-file-per-invocation contract from inside the stub `shellcheck` (via `STUB_ARGC_LOG`), because a batched run and a per-file run print the *same* verdict and nothing in the stage's own output can tell them apart; section **M2** covers the watchdog wiring on the CI path specifically - an OVER BUDGET kill and a HOST MEMORY PRESSURE kill both name their cause and both fail the stage, and neither is ever reported as `SKIPPED`.

**The `macos-latest` row above is now HISTORICAL - it no longer runs at all.**
CI run `35298522350` (`dev` @ `6c15cd7`) measured a real macOS runner at "7GB total, **2GB** available, 2GB reserved -> **1GB** headroom" - worse than the ~5GB available/3GB headroom this section's own table assumes - and every one of the ten `macos-latest` shards failed, killing ORDINARY files (`modules/image/run.sh`, `tests/suites/dast-markup.sh` - neither on `tests/shellcheck-heavy-files.txt`) under a ceiling that cannot fit even one file at the ~6GB `SCOURSH_SHELLCHECK_CI_WORST_GB` plans per file. There is no per-file tuning that fits a ~6GB-per-file plan into 1GB of headroom.
The stage now takes a **declared platform handoff** instead: on a GitHub-Actions runner where `RUNNER_OS == macOS`, `sc_stage` (`tests/run-tests.sh`) skips the arithmetic above entirely, attempts zero files, and prints so unconditionally (`DECLARED PLATFORM HANDOFF`, `0 file(s) checked on this runner`) before returning a pass - the same "declared, never silent" discipline `tests/shellcheck-heavy-files.txt`'s own CI-to-daily-suite handoff already uses. This costs no coverage: `shellcheck` is static analysis, so the identical pinned v0.11.0 binary (`.github/workflows/ci.yml` installs the same build on both legs) reading the same bytes yields the same findings regardless of host userland - unlike the *suites*, which this project genuinely runs on both GNU and BSD to catch runtime behavioural differences (`docs/FOUNDATION.md` tension 24). `ubuntu-latest` still checks the full, unfiltered file set (minus the separately-declared heavy-file handoff to `tools/daily-suite.sh`'s GNU leg), so nothing goes unchecked.
The gate is `RUNNER_OS`, the variable GitHub Actions itself sets, never `uname` - a contributor's own real Mac has real headroom and still gets a real, local `shellcheck` run.
`tests/suites/run-tests-stage.sh` section **O** proves the handoff is a true no-op (zero `shellcheck` invocations, measured via the same `STUB_ARGC_LOG` technique section M uses - never a return value) and that it is declared in the log; the union-completeness case in the `--shard I/N` section extends the same proof over the real, full-tree file list and confirms macOS contributes zero files to it.
...
**A whole class of fan-out was invisible to `tests/lint-source-graph.sh`'s own walker, and it was found by watching `shellcheck -x` follow an edge the walker swore did not exist.** The walker (correctly) mirrors shellcheck's documented directive rule: a `# shellcheck source=<path>` comment applies only to the `source`/`.` line immediately after it, comments aside, and a `source "$VAR/..."` line with no such directive is not a followed edge. What it does not mirror is shellcheck's OWN heuristic: when the sourced path's directory portion is derived from `BASH_SOURCE`/`$0` (the common `ROOT=$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")/.." && pwd -P)` idiom, or an inline `${BASH_SOURCE[0]%/*}/...`), shellcheck resolves it anyway and follows the edge for real - with no directive, no `SC1091`, and no entry in the walker's graph. A `source "$ROOT/modules/dast/run.sh"` line in `tests/suites/dast.sh`, sitting three lines below an *existing* `# shellcheck source=/dev/null` cut for the same content one call up, had exactly this shape: hub sum for the file read **0-2** (only `tests/lib/assert.sh`, since the declared edge was cut), while `shellcheck -x` itself measured **~20-22 GB** on this file and was OOM-killed past **30 GB** in a 16 GB-class Linux container. Diagnosed by bisection (`head -n N file | shellcheck -x -s bash -`, halving `N` until the run stopped taking minutes), not by reading the graph, because the graph had nothing to show. The same shape recurred five more times: a fifth, unguarded `source modules/dast/jwt.sh` call in `tests/suites/dast-jwt.sh` (its four siblings already carried the correct `/dev/null` cut - this one was just missed when it was added); `tests/suites/network-tlsport.sh` and `tests/suites/network-transport.sh`, each sourcing their own module under test once per test case (5-6 times) with zero directives anywhere, **plus** their own top-level direct edges to `lib/http.sh`, `modules/network/reachability_engine.sh`, `lib/nettransport.sh` and `modules/dast/passive/tls_engine.sh` duplicating content the module-under-test's own real edge (once made explicit) already supplies; `tests/suites/image.sh`'s one `modules/image/run.sh` call; and a **placement** bug in production code, `modules/network/transport_engine.sh`, where a real `# shellcheck source=modules/dast/passive/tls_engine.sh` directive sat before an `if [[ ... ]]; then` guard rather than directly above the `source` line one level down - never applied per the walker's own rule, and (being `${BASH_SOURCE[0]%/*}/...`-shaped) followed anyway by shellcheck's heuristic regardless. Every fix follows the existing convention exactly: name the edge with a real directive where it is the first/only occurrence, or where a directive was merely misplaced; cut it to `/dev/null` where the target is already inlined elsewhere in the same file's graph. **The fix makes the cost visible and trackable, not necessarily small.** `tests/suites/network-tlsport.sh` and `network-transport.sh` went from a falsely-clean hub sum of 13 (missing the bug entirely) to a briefly-worse 20/21 (once the real edge was named and its now-duplicate top-level siblings still stood) back down to 13/14 once those siblings were cut to `/dev/null` - the SAME hub sum as before, but now with zero duplicate expansions rather than one uncounted one. That is a genuine, provable improvement in the graph, and it is not the same claim as "now fits a 16 GB runner": a real `shellcheck -x` run on the fixed `network-tlsport.sh` (hub sum 13, no duplicates, unchanged from what this tree has carried since before this fix) was *also* observed OOM-killed at this same container's ~15 GB ceiling. `tests/suites/dast.sh` and `modules/dast/run.sh`'s own graph (hub sum 7, no duplicates once named) hit the identical ceiling. Read that as a second, separate finding rather than a failure of the fix: **shellcheck's own non-determinism (documented above - "peak RSS is not a fixed property of a file") reaches down to hub sums well below the cap**, not only to `dast-methods.sh` at the cap's own edge (whose CI comment records the identical shape - 6.43 GB on one runner, >14 GB and still climbing on a 64 GB host, same commit). A hub sum that undercounts (this bug) is always a defect to fix, and fixing it is what turns an *unbounded* cost into a *bounded, measured* one - the difference between "was OOM-killed past 30 GB" and "was OOM-killed at this container's own ~15 GB ceiling" for `dast.sh` is real and is the entire benefit fan-out reduction can offer here. But a hub sum that is already minimal and still occasionally exceeds a 16 GB-class runner is evidence of shellcheck's own ambient-memory volatility, not a remaining fan-out defect - out of this discipline's remit per its own header ("this cap is what keeps that MAX inside a runner... it adds one back-edge cut on an edge whose target the entry point ALREADY reaches another way"), and a case for the stage's existing SKIP outcome (see "Skipped is a third outcome" below) rather than for a lower cap or a further cut that does not exist.
...
It is a *stage*, not a suite or a linter file - it has no `tests/*.sh` of its own - so it is listed under `stages:` in `--list` and named in `STAGES`, not `SUITES`/`LINTERS`. That named-target invocation is always unsharded and always checks the whole tree.
`SCOURSH_SHELLCHECK_FILE_LIST` overrides the file list, and now has two legitimate callers: `tests/suites/run-tests-stage.sh` (a **test seam**, driving the real stage over a two-file fixture with a stub `shellcheck` so the watchdog and the signal paths are exercised in seconds) and `tests/run-tests.sh --shard I/N` itself (a **real** caller as of the file-list sharding change below - a shard sets it internally to exactly the files it owns before making its one `sc_stage` call). It is still not a way to exclude files from an *unsharded* run: `tests/run-tests.sh` and `tests/run-tests.sh shellcheck` both leave it unset and both still check every `*.sh` in the tree.

**The stage's FILE LIST is sharded across `--shard I/N`, one `file:<relpath>` item per file, rather than the whole stage being pinned to one shard.** CI run 35167252242 (dev, ten shards, both userlands) had zero test failures - 18 of 20 jobs green - and the two shards that drew the (then-monolithic) stage were both cancelled at the 150-minute ceiling mid-way through the stage's own pass 2, while nineteen other jobs sat idle. A file's weight is its `tests/lint-source-graph.sh` hub sum (shared via `tests/lib/hubsum.sh`), cubed - real, self-updating signal rather than a hardcoded list of "today's heavy files" - so the tree's genuinely expensive files spread across shards instead of queueing behind each other on one. See `tests/run-tests.sh`'s own `--shard` header ("The shellcheck STAGE is not one item in that plan - its FILE LIST is") for the full mechanism, and AGENTS.md's `--shard` bullets for the summary.
