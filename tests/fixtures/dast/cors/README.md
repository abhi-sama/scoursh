...
| `redirect-reflect.headers` | A 3xx with `Location:` a DIFFERENT origin, which ALSO reflects the sentinel on this (unfollowed) response - `cors_probe` sends with `max_redirects` 0, so this is never followed, and the finding must stay anchored to the REQUESTED origin's own canonical form, never adopt the `Location`'s |
