...
`scoursh` target-agnostic, so every host in this directory is invented. Product strings are either
invented (`fixtureserver`, `fixturelang`) or common public components (WordPress, jQuery) used to
exercise product normalisation; none of them names a target.
