...
- **A `# -x back-edge cut: X is already inlined elsewhere in this file's own source graph` comment is a CLAIM, and the tree already had two that were wrong** (`modules/dast/passive/leakage_engine.sh` and `transport_engine.sh`, each cutting their own `lib/http.sh` edge on that premise). Neither of leakage_engine.sh's two real consumers (`modules/dast/passive/leakage.sh`, `tests/suites/dast-leakage.sh`) - nor transport_engine.sh's two - has ANY other real edge to `lib/http.sh` anywhere in their followed graph, so `shellcheck -x` had zero static knowledge of it for either entry point, and `SCOURSH_HTTP_RESOLVE`/`SCOURSH_HTTP_TRANSPORT` read a real `SC2034` ("appears unused") at their first assignment - invisible until `tests/lint-source-graph.sh`'s own walker was used to check hub reachability directly (`grep`-driven manual tracing of a multi-hop `source=` chain is exactly the kind of thing that reads right and is wrong). The fix restored both to real edges, matching `modules/dast/passive/headers_engine.sh`'s own (correct) convention for the identical shape. `tests/suites/dast-transport.sh` carried a SECOND instance of the same mistake one level up: its own top-level `source transport_engine.sh` was ALSO cut, on the claim that sourcing `modules/dast/engine.sh` pulls in `headers_engine.sh` -> `lib/http.sh` - which `modules/dast/engine.sh` does not do and, per `git blame`, never did. **Trust the lint's reachability count over the comment**; a stale or simply wrong justification for a cut is not itself evidence the cut is safe.
- **`shellcheck -x`'s SC2034 has a narrower dead-STORE mode alongside its ordinary "never read anywhere" one, and it explains a two-assignment file showing exactly one finding.** `tests/suites/dast-leakage.sh` assigns `SCOURSH_HTTP_RESOLVE` twice (a stub swapped mid-file for a second test phase); only the FIRST assignment was flagged. The check does not require the name be unread everywhere - it flags a SPECIFIC assignment already provably dead because no textual read of that name appears before the NEXT assignment overwrites it, and the final assignment in a file has no "next" to be judged dead against. Do not read a lone, un-followed-up SC2034 next to an apparently-identical second assignment as "shellcheck is inconsistent, ignore it" - it usually means the FIRST assignment's value really is never used, which is a shape worth double-checking rather than suppressing on sight.
- **The established fix for `shellcheck -x` conflating two functions' same-named locals across different files is a DISTINCT NAME, never a suppression** (`tests/suites/dast-sqli.sh`'s own `_sqli_now` comment records this first). `-x` inlines every sourced file into one flat namespace, so a plain `local kept=1` in `modules/dast/ratelimit.sh` collided with two unrelated functions' `local -a kept=()` in `lib/http.sh` and tripped `SC2178` ("used as an array but is now assigned a string"); `tests/suites/dast-cmdi.sh`'s `local cur=0 pend=0` collided the same way with `local -A cur=()`, an idiom five different `modules/dast/**/*.sh` engines already share. Renaming to `endpoint_in_scope` and `cur_ns`/`pend_ns` fixed both. A `# shellcheck disable=SC2178` would silence the real case too, the first time a same-named array genuinely reaches this scope - which is exactly the failure mode a suppression is supposed to prevent, not cause.
