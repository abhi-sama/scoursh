...
# hand-typed statements of the same fact, so the two can never disagree about
...
# -----------------------------------------------------------------------------
# 2b. Per-subcommand help (`scan.sh <command> --help`).  Nothing here
#     hand-copies scan_usage's grammar: the accepted-flags list below is
#     generated by walking `_SCAN_FLAG_KIND`, the exact map `scan_flag_kind`
#     itself consults to accept or reject a real flag, so this text can never
#     list a flag the parser would refuse, or omit one it accepts.
#
#     The one line that IS prose - "built" versus "not built" - is answered
#     by a real on-disk check wherever this project has one: `_scan_module_built`
#     below, in scan_usage_for's own `case $cmd` block.  Where it does not
#     (`diff`/`report`, which are not modules and have no `run.sh` to check),
#     `_scan_stateful_command_built` is the SAME function scan_main's own
#     dispatch reads, so the two can never silently disagree - see AGENTS.md's
#     "Sharp edges" entry on this for why a file-existence proxy is not
#     available for those two.
# -----------------------------------------------------------------------------

# `_scan_module_built CMD` - exactly the check `scan_dispatch` itself makes
# (section 7 below) before deciding whether to source a real module or log a
# `not_yet_built` coverage_reduction.  Reusing it here rather than a second,
# separately-typed judgement is what makes this help text unable to claim a
# module is built when a real invocation would then find no run.sh.
_scan_module_built() {
  [[ -f $(_scan_module_script "$1") ]]
}

# `_scan_dast_phase_status` - "N of M scan phases implemented", counted by
# walking modules/dast/engine.sh's own `_DAST_PHASES` table and checking the
# same file path `dast_run_phase` checks for each row (its "absent" outcome).
# Sourcing engine.sh costs nothing at source time (a pure function/array
# library, guarded, already safe to source more than once - see its own
# header) and is skipped entirely if modules/dast/run.sh itself does not
# exist, so a fixture SCOURSH_INSTALL_ROOT with no dast module at all never
# tries to source a file that is not there.
_scan_dast_phase_status() {
  local total=0 present=0 spec script path
  if [[ -z ${_DAST_PHASES+x} && -f $SCOURSH_INSTALL_ROOT/modules/dast/engine.sh ]]; then
    # shellcheck disable=SC1091
    source "$SCOURSH_INSTALL_ROOT/modules/dast/engine.sh"
  fi
  for spec in "${_DAST_PHASES[@]+"${_DAST_PHASES[@]}"}"; do
    script=${spec%%:*}
    path=$SCOURSH_INSTALL_ROOT/modules/dast/$script
    total=$(( total + 1 ))
...
scan_usage_for() {
  local cmd=$1 key flag kind req
  local -a own_flags=() global_flags=()

  for key in "${!_SCAN_FLAG_KIND[@]}"; do
    case $key in
      "$cmd":*) own_flags+=("${key#*:}") ;;
      global:*) global_flags+=("${key#global:}") ;;
    esac
  done
  ...
  if (( ${#own_flags[@]} > 0 )); then
    ...
    while IFS= read -r flag; do
      [[ -n $flag ]] || continue
      kind=$(scan_flag_kind "$cmd" "$flag")
      if [[ $kind == bool ]]; then
        printf '  --%s\n' "$flag"
      else
        printf '  --%s VALUE\n' "$flag"
      fi
    done < <(printf '%s\n' "${own_flags[@]+"${own_flags[@]}"}" | LC_ALL=C sort)
    printf '\n'
  fi

  printf 'Global flags (accepted by every command - see scan.sh --help for details):\n'
  while IFS= read -r flag; do
    [[ -n $flag ]] || continue
    kind=$(scan_flag_kind global "$flag")
    if [[ $kind == bool ]]; then
      printf '  --%s\n' "$flag"
    else
      printf '  --%s VALUE\n' "$flag"
    fi
  done < <(printf '%s\n' "${global_flags[@]+"${global_flags[@]}"}" | LC_ALL=C sort)

  printf '\nStatus: '
  case $cmd in
    sast | sca | iac)
      if _scan_module_built "$cmd"; then
        printf '%s\n' 'built.'
      else
        printf '%s\n' "NOT built - modules/$cmd/run.sh does not exist on disk yet."
      fi
      ;;
    dast)
    ...
    report)
      ...
      ;;
    all)
      printf '%s\n' 'runs every module above for which its required inputs were given; any module that is not yet built is skipped with its own declared coverage_reduction, exactly as running it standalone would be.'
      ;;
    *)
      printf '%s\n' 'unknown command'
      ;;
...
scan_parse_args() {
  ...
  # _SCAN_REQUIRED_FLAG (section 2 above) is the single source of truth for
  # which commands need a flag no other flag can stand in for; scan_usage_for
  # reads the same map, so the two can never state a different requirement.
  local _scan_req=${_SCAN_REQUIRED_FLAG[$SCAN_COMMAND]:-}
  if [[ -n $_scan_req ]]; then
...
# `_scan_capture_list VARNAME CMD [ARGS...]` - the same subshell-swallows-die()
# hazard as `_scan_capture` above, for a command that prints MULTIPLE lines
# (config_scanner_list's `formats`/`paranoid-allow`) rather than one.  Joins
# the captured lines into VARNAME as a comma-separated list, which is the
# shape every multi-cardinality scanner.conf key in this project already
# takes on the way in (`--format json,sarif,html,md`), so a caller can feed
# VARNAME straight back into another CSV consumer.
_scan_capture_list() {
  local __var=$1
  shift
  local __tmp=$SCOURSH_SCRATCH/_scan_capture_list.$$
  "$@" >"$__tmp"
  local __line __joined=''
  while IFS= read -r __line || [[ -n $__line ]]; do
    [[ -n $__line ]] || continue
    __joined=${__joined:+$__joined,}$__line
  done <"$__tmp"
  rm -f "$__tmp"
  # shellcheck disable=SC2229
...
scan_main() {
  ...
  # SCOURSH_FORMATS: the resolved --format list, exported so every module's
  # own report_all call (lib/report.sh) - not just this function's own,
  # redundant final report_run_json below - honours what was actually asked
  # for.  Resolved through the same CLI>env>file>default chain as every other
  # scanner.conf key; the default with no --format given is every format
  # (lib/config.sh's `_scanner_default_list formats`), which is today's
  # unchanged behaviour for every caller that has never passed --format.
